...
class Model():
    ...
    def train_epoch(self, epoch):
        print("--------------------start training epoch %02d--------------------" % epoch)
        for iter_, (images, actions, states) in enumerate(self.dataloader['train']):
            ...
            if iter_ % self.opt.print_intercal == 0:
                print("training epoch: %03d, iterations: %06d loss: %06f" % (epoch, iter_, loss))
        ...
        with torch.no_grad():
            ...
            for iter_, (images, actions, states) in enumerate(self.dataloader['valid']):
                ...
                for i, (state, gen_state) in enumerate(
                    ...
                    state_loss += self.mse_loss(state, gen_state) * self.w_state
            recon_loss /= torch.tensor(self.opt.sequence_length - self.opt.context_frames) * len(self.dataloader['valid'])
            state_loss /= torch.tensor(self.opt.sequence_length - self.opt.context_frames) * len(self.dataloader['valid'])

            print("evaluation epoch: %03d, recon_loss: %06f, state_loss: %06f" % (epoch, recon_loss, state_loss))
