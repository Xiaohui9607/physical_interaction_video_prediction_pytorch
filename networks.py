...
class ConvLSTM(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=5, forget_bias=1.0):
        super(ConvLSTM, self).__init__()
        self.out_channels = out_channels
        self.conv = nn.Conv2d(in_channels=2 * in_channels, out_channels=4 * out_channels, kernel_size=kernel_size, stride=2)
        self.forget_bias = forget_bias

    def forward(self, inputs, states):
        if states is None:
            states = (torch.zeros_like(inputs), torch.zeros_like(inputs))
        assert isinstance(states, tuple)

        c, h = states
        assert len(c.shape) == 4 and len(h.shape) == 4 and len(inputs.shape) == 4

        inputs_h = torch.cat((inputs, h), dim=1)
        i_j_f_o = self.conv(inputs_h)
        i, j, f, o = torch.split(i_j_f_o,  self.out_channels, dim=2)
...
class network(nn.Module):
    def __init__(self, channels=3,
        ...
        self.enc0 = nn.Conv2d(in_channels=channels, out_channels=32, kernel_size=5, stride=2)
        self.enc0_norm = nn.LayerNorm(32)
        # N * 32 * H/2 * W/2 -> N * 32 * H/2 * W/2
        self.lstm1 = ConvLSTM(in_channels=32, out_channels=lstm_size[0], kernel_size=5)
        self.lstm1_norm = nn.LayerNorm(lstm_size[0])
        # N * 32 * H/2 * W/2 -> N * 32 * H/2 * W/2
        self.lstm2 = ConvLSTM(in_channels=lstm_size[0], out_channels=lstm_size[1], kernel_size=5)
        self.lstm2_norm = nn.LayerNorm(lstm_size[1])

        # N * 32 * H/4 * W/4 -> N * 32 * H/4 * W/4
        self.enc1 = nn.Conv2d(in_channels=lstm_size[1], out_channels=lstm_size[1], kernel_size=3, stride=2)
        # N * 32 * H/4 * W/4 -> N * 64 * H/4 * W/4
        self.lstm3 = ConvLSTM(in_channels=lstm_size[1], out_channels=lstm_size[2], kernel_size=5)
        self.lstm3_norm = nn.LayerNorm(lstm_size[2])
        # N * 64 * H/4 * W/4 -> N * 64 * H/4 * W/4
        self.lstm4 = ConvLSTM(in_channels=lstm_size[2], out_channels=lstm_size[3], kernel_size=5)
        self.lstm4_norm = nn.LayerNorm(lstm_size[3])
        # pass in state and action

        # N * 64 * H/4 * W/4 -> N * 64 * H/8 * W/8
        self.enc2 = nn.Conv2d(in_channels=lstm_size[3], out_channels=lstm_size[3], kernel_size=3, stride=2)
        ...
        self.lstm5 = ConvLSTM(in_channels=lstm_size[3], out_channels=lstm_size[4], kernel_size=5)
        self.lstm5_norm = nn.LayerNorm(lstm_size[4])
        # N * 128 * H/8 * W/8 -> N * 128 * H/4 * W/4
        self.enc4 = nn.ConvTranspose2d(in_channels=lstm_size[4], out_channels=lstm_size[4], kernel_size=3, stride=2)
        # N * 128 * H/4 * W/4 -> N * 64 * H/4 * W/4
        self.lstm6 = ConvLSTM(in_channels=lstm_size[4], out_channels=lstm_size[5], kernel_size=5)
        self.lstm6_norm = nn.LayerNorm(lstm_size[5])

        # N * 64 * H/4 * W/4 -> N * 64 * H/2 * W/2
        self.enc5 = nn.ConvTranspose2d(in_channels=lstm_size[5], out_channels=lstm_size[5], kernel_size=3, stride=2)
        # N * 64 * H/2 * W/2 -> N * 32 * H/2 * W/2
        self.lstm7 = ConvLSTM(in_channels=lstm_size[5], out_channels=lstm_size[6], kernel_size=5)
        self.lstm7_norm = nn.LayerNorm(lstm_size[6])
        # N * 32 * H/2 * W/2 -> N * 32 * H * W
        self.enc6 = nn.ConvTranspose2d(in_channels=lstm_size[6], out_channels=lstm_size[6], kernel_size=3, stride=2)
        self.enc6_norm = nn.LayerNorm(lstm_size[6])
    ...
    def forward(self, images, init_state, actions):
        '''
        ...
        if self.k == -1:
            ...
            num_ground_truth = torch.round(images.shape[0] * (self.k / math.exp(self.iter_num/self.k) + self.k)).int()
            feedself = False
        ...
        for image, action in zip(images[:-1], actions[:-1]):
            ...
            if feedself and done_warm_start:
                ...
                # Scheduled sampling
                image = self.scheduled_sample(image, gen_images[-1], images.shape[0], num_ground_truth)
            else:
                # Always feed in ground_truth
                image = image

            enc0 = self.enc0_norm(F.relu(self.enc0(image)))
            ...
            enc2 = F.relu(self.enc2(lstm4))

            # pass in state and action
            state_action = torch.cat([action, current_state], dim=1)
            smear = torch.reshape(state_action, state_action.shape+[1, 1])
            ...
            enc6 = self.enc6_norm(F.relu(self.enc6(lstm7)))
            ...
            masks = F.relu(self.maskout(enc6))
            masks = F.softmax(masks.view(self.num_masks+1))
    ...
    def cdna_transformation(self, image, cdna_input):
        ...
        transformed = F.conv2d(image, cdna_kerns, stride=1, padding=[2,2], groups=batch_size)
    ...
    def dna_transformation(self, image, dna_input):
        ...
        kernel = F.relu(dna_input-RELU_SHIFT)+RELU_SHIFT
    ...
    def scheduled_sample(self, ground_truth_x, generated_x, batch_size, num_ground_truth):
        ...
        generated_examps = torch.stack([ground_truth_x[idx[:num_ground_truth]], generated_x[idx[num_ground_truth:]]])
